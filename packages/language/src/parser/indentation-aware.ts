/**
 * This file is a copy of the `indentation-aware.ts` file from the `langium` repository until it gets published in v3.2.0.
 * It also currently contains some modifications that are not present in the original file yet,
 * with the intention to submit pull requests of the added features after testing.
 */

import type { CustomPatternMatcherFunc, TokenType, IToken, IMultiModeLexerDefinition } from 'chevrotain';
import type { LangiumCoreServices, LexerResult, TokenBuilderOptions, GrammarAST } from 'langium';
import { createToken, createTokenInstance, Lexer } from 'chevrotain';
import { DefaultTokenBuilder, DefaultLexer, isTokenTypeArray } from 'langium';
...
export const REGULAR_MODE = 'indentation-aware';
export const IGNORE_INDENTATION_MODE = 'ignore-indentation';
